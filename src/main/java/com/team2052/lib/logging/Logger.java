...
public class Logger {
  ...
  private static Map<String, Pair<StructLogEntry<StructSerializable>, StructSerializable>>
  ...
  public static void log(String name, double value) {
    name = "Data Logger/" + name;
    System.out.println("Logging " + name + ": " + value + " at time " + Timer.getFPGATimestamp());
    if (!doubleEntries.containsKey(name)) {
      DoubleLogEntry entry = new DoubleLogEntry(DataLogManager.getLog(), name);
      System.out.println("Created new log entry for " + name);
      entry.append(value);
    ...
    entryPair.getFirst().append(value);
    System.out.println("Updating log entry for " + name);
  ...
  public static <T extends StructSerializable> void log(String name, T value) {
    ...
    if (!structEntries.containsKey(name)) {
      ...
      switch (value.getClass().getSimpleName()) {
        case "Pose2d":
          entry =
              (StructLogEntry<T>)
                  StructLogEntry.create(DataLogManager.getLog(), name, Pose2d.struct);
          break;
    ...
    Pair<StructLogEntry<StructSerializable>, StructSerializable> entryPair =
        structEntries.get(name);
    if (entryPair.getSecond().equals(value)) {
      return;
    }
